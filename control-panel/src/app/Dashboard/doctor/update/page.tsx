...
export default function UpdateDoctor({
  ...
  const { data } = useGetDoctorById(id)
  ...
  useEffect(() => {
    if (data) {
      const { doctorName, specialist, phone, department, weekwork } = data;
  ...
  const mutation = useMutation({
    ...
    mutationFn: (data: doctroType) => doctroUpdate(data, id), // استخدم الدالة المعدلة
    onError: (err: any) => {
    ...
    <div dir="rtl">
      <Modal open={isUpdating} closeIcon={false} footer={null}>
        <div className="text-2xl font-bold mb-4 font-serif text-center">
          ...
          <h1 className="text-red-700">{data?.doctorName}</h1>
        </div>
        <form
          dir="rtl"
